...
/**
 * Attach the matching playbook position and suggested response to each deviation.
 * Both are null when no entry matches — callers supply their own placeholder
 * (the findings table on screen, the fallback text in the .docx export).
 */
export function augmentWithPlaybook(deviations = [], entries = []) {
  return deviations.map((d) => {
    const match = findPlaybookMatch(d.clauseName, entries)
    return {
      ...d,
      playbookPosition: match?.preferredPosition || null,
      suggestedResponse: match?.suggestedResponse || null,
...
const CLAUSE_STOPWORDS = new Set(['of', 'and', 'or', 'the', 'a', 'to', 'for', 'in'])

/**
 * Find the playbook entry that covers a clause block from a document, so the
 * firm's position can be sent to the model alongside the clause itself.
 * Matches on the block's heading first, then falls back to looking for every
 * significant word of a clause type in the heading and opening text.
 */
export function findPlaybookMatchForBlock(label, text, entries) {
  if (!entries?.length) return null

  const byHeading = findPlaybookMatch(label, entries)
  if (byHeading) return byHeading

  const haystack = `${label || ''} ${String(text || '').slice(0, 400)}`.toLowerCase()
  return (
    entries.find((e) => {
      const words = e.clauseType
        .toLowerCase()
        .split(/[^a-z0-9]+/)
        .filter((w) => w && !CLAUSE_STOPWORDS.has(w))
